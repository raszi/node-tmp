...
const prefixStackStorage = new asyncHooks.AsyncLocalStorage();
let
...
async function withSubdir(prefix, fn, options) {
  return new Promise((resolve, reject) => {
    ...
    prefixStackStorage.run({ value: tmpDir }, () => {
      prefixStackStorage.getStore().value = tmpDir;
      fn(cleanup).then(resolve, reject)
        .finally(() => {
          if (options && options.cleanup) return cleanup();
        });
    });
  });
}
