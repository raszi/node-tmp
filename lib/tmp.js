...
function _createReadStream(tmpObject, cb) {
    ...
    if (typeof cb === 'function')
        try {
            cb(null, fs.createReadStream(tmpObject.name, options));
        }
        catch (err) {
            cb(err, null);
        }   
    else {
        return fs.createReadStream(tmpObject.name, options);
    }
}
...
function _createWriteStream(tmpObject, cb) {
    ...
    var options = {
      fd : tmpObject.fd,
      flags : _c.O_APPEND | _c.O_CREAT,
      mode : FILE_MODE
    };

    if (typeof cb === 'function')
        try {
            cb(null, fs.createWriteStream(tmpObject.name, options));
        }
        catch (err) {
            cb(err, null);
        }   
    else {
        return fs.createWriteStream(tmpObject.name, options);
    }
}

/**
function _assertTmpObject(tmpObject) {
    if (typeof tmpObject !== 'object' ||
        typeof tmpObject.name !== 'string' ||
        typeof tmpObject.fd !== 'number')
        throw new Error('tmpObject must have both a name and an fd property.');
}
...
module.exports.createReadStream = _createReadStream;
module.exports.createWriteStream = _createWriteStream;
